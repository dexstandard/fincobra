...
export default async function twofaRoutes(app: FastifyInstance) {
    app.get(
        '/2fa/status',
        { config: { rateLimit: RATE_LIMITS.MODERATE }, preHandler: sessionPreHandlers },
        async (req) => {
    ...
    app.get(
        '/2fa/setup',
        { config: { rateLimit: RATE_LIMITS.TIGHT }, preHandler: sessionPreHandlers },
        async (req) => {
            const userId = getValidatedUserId(req);
    ...
    app.post(
        '/2fa/enable',
        { config: { rateLimit: RATE_LIMITS.VERY_TIGHT }, preHandler: sessionPreHandlers },
        async (req, reply) => {
            const userId = getValidatedUserId(req);
