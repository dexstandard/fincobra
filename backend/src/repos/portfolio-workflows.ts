...
const baseSelect = `
  SELECT pw.id, pw.user_id, pw.model, pw.status, pw.created_at, pw.start_balance, pw.cash_token,
         ...
         pw.risk, pw.review_interval, pw.agent_instructions, pw.manual_rebalance, pw.use_earn, pw.trade_mode,
...
export async function findIdenticalInactiveWorkflow(
  ...
  const query = `SELECT pw.id FROM portfolio_workflow pw
    ...
    WHERE pw.user_id = $1 AND pw.status = 'inactive' AND ($2::bigint IS NULL OR pw.id != $2)
      ...
      AND pw.risk = $5 AND pw.review_interval = $6 AND pw.agent_instructions = $7 AND pw.manual_rebalance = $8 AND pw.use_earn = $9 AND pw.trade_mode = $10
...
export async function findActiveTokenConflicts(
  ...
  const params: unknown[] = [userId, excludeId ?? null, tokens, 'spot'];
  const { rows } = await db.query(query, params as any[]);
...
export async function insertPortfolioWorkflow(
  ...
  await withTransaction(async (client) => {
    const { rows } = await client.query(
      `INSERT INTO portfolio_workflow (user_id, model, status, start_balance, cash_token, risk, review_interval, agent_instructions, manual_rebalance, use_earn, trade_mode)
...
export async function updatePortfolioWorkflow(
  ...
  await withTransaction(async (client) => {
    await client.query(
      `UPDATE portfolio_workflow SET model = $1, status = $2, cash_token = $3, risk = $4, review_interval = $5, agent_instructions = $6, start_balance = $7, manual_rebalance = $8, use_earn = $9, trade_mode = $10 WHERE id = $11`,
      [
        data.model,
        data.status,
        data.cashToken,
