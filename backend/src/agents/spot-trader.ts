...
export const spotRebalanceResponseSchema = {
  type: 'object',
  properties: {
    result: {
      anyOf: [
...
const pendingNewsContexts = new Map<string, Promise<SpotNewsContext>>();

function getCachedNewsContext(token: string, now: number): SpotNewsContext | null {
...
async function buildStablecoinOracleReport(
  ...
  try {
    ...
    const pair: SpotStablecoinOracleReport['pair'] =
      symbol === 'USDT' ? 'USDT/USD' : 'USDC/USD';
    const quote = await getUsdPrice(symbol);
    const quotePayload: SpotStablecoinOracleQuoteReport = {
...
export function __resetSpotTraderCachesForTest(): void {
  newsContextCache.flushAll();
  pendingNewsContexts.clear();
}

export function clearSpotTraderCaches(): void {
  newsContextCache.flushAll();
  pendingNewsContexts.clear();
  clearMarketOverviewCache();
}

function createEmptyNewsContext(): SpotNewsContext {
  return {
...
export async function collectSpotPromptData(
  ...
  const positions: SpotRebalancePosition[] = [];
  const routes: SpotRebalancePrompt['routes'] = [];
  ...
  const portfolio: SpotRebalancePrompt['portfolio'] = {
  ...
  const previousReports: SpotPreviousReport[] = [];
  for (let index = 0; index < prevRows.length; index += 1) {
    ...
    const orders: SpotPreviousReportOrder[] = [];
    for (const o of ordersRows) {
      ...
      const order: SpotPreviousReportOrder = {
    ...
    if (r.log) {
      try {
        const parsedLog = JSON.parse(r.log) as Partial<SpotTraderDecision>;
    ...
    const report: SpotPreviousReport = {
  ...
  const reports: SpotPromptReport[] = [];
  ...
  const prompt: SpotRebalancePrompt = {
...
function extractResult(
  ...
  res: string,
): SpotTraderDecision | null {
  const parsed = extractAiJson<{ result?: SpotTraderDecision }>(provider, res);
  if (!parsed) return null;
  return parsed.result ?? null;
}

export async function runSpotTrader(
  { log, model, apiKey, aiProvider }: RunParams,
  prompt: SpotRebalancePrompt,
  instructionsOverride?: string,
): Promise<SpotTraderDecision | null> {
