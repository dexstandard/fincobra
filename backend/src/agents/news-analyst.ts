...
export const newsAgentResponseSchema = {
  ...
  properties: {
    ...
    time_sensitivity: { type: 'number', minimum: 0, maximum: 1 },
...
export function buildNewsAgentUserContent(input: NewsAgentInput): string {
  ...
  if (tokens) {
    payload.referenced_symbols = tokens;
  }
  if (input.sentimentHint) {
    payload.sentiment_hint = input.sentimentHint;
  }
  if (input.categoryHint) {
    payload.category_hint = input.categoryHint;
  }
...
export function parseNewsAgentContent(content: string): NewsAgentAnalysis {
  ...
  const categoryConfidence = clampUnit(
    requireNumber(raw.category_confidence, 'category_confidence'),
  );
  const sentimentConfidence = clampUnit(
    requireNumber(raw.sentiment_confidence, 'sentiment_confidence'),
  );
  ...
  const timeSensitivity = clampUnit(
    requireNumber(raw.time_sensitivity, 'time_sensitivity'),
  );
  ...
  const riskNotes = optionalString(raw.risk_notes);
